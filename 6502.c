...
void brk6502() {
      	PC++;
	ram[0x0100+S--] = (uint8_t)(PC>>8);
	ram[0x0100+S--] = (uint8_t)(PC & 0xff);
	ram[0x0100+S--] = P;
	P |= 0x14;
...
void tsb6502() {
      	adrmode[opcode]();
      	ram[savepc] |= A;
      	if (ram[savepc]) P &= 0xfd; else P |= 0x02;
}

void trb6502() {
      	adrmode[opcode]();
      	ram[savepc] = ram[savepc] & (A ^ 0xff);
      	if (ram[savepc]) P &= 0xfd; else P |= 0x02;
}
/* Reset CPU */
void reset6502() {
